...
var enemyzombieLayer = cc.Layer.extend({
   ctor: function() {
      this._super();
...
var EnemyZombie = cc.Sprite.extend({
  ctor: function() {
    ...
    var animationframe = [];
    //スプライトフレームを格納する配列
    var texture = cc.textureCache.addImage(res.zombie_frames);
    /*for (i = 0; i < 4; i++) {
        //スプライトフレームを作成
        var frame = new cc.SpriteFrame.createWithTexture(texture, cc.rect(128 * i, 128, 128, 128));
        //スプライトフレームを配列に登録
        animationframe.push(frame);
    }
    for (j = 0; j < 2; j++) {
      //スプライトフレームを作成
      var frame = new cc.SpriteFrame.createWithTexture(texture, cc.rect(128, 128 * j, 128, 128));
      //スプライトフレームを配列に登録
      animationframe.push(frame);
    }*/
    for (i = 0; i < 2; i++) {
